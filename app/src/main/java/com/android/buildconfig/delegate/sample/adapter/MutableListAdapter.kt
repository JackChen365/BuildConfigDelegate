 ...
 * @see swapList
 * This is for us to do some undo operation. We just keep the old snapshot and compare with the new one.
 *
 * <p>
 *     fun undo() {
 *          val newList = oldSnapshot
 *          if (null != newList) {
 *          val oldList = snapshot()
 *          //Swap list without sync the diff to the View.
 *          swapList(newList)
 *          //dispatch the update to the adapter.
 *          calculateDiff(oldList, newList, ::compareItems).dispatchUpdatesTo(this)
 *      }
 *
 *      override fun onCalculateDiff(oldList: List<String>, newList: List<String>) {
 *          super.onCalculateDiff(oldList, newList)
 *          oldSnapshot = oldList
 *      }
 * </p>
 *
